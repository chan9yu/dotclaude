...
└── statusline.sh        # 상태바 스크립트 (시각, 모델, Git 상태)
```
...
**statusline.sh** — 터미널 상태바에 현재 시각, 모델명, 디렉토리, Git 브랜치/상태를 표시한다.
...
`settings.json`의 `mcpServers` 경로는 로컬 환경에 맞게 수정한다. `ide/`, `plugins/`, `skills/`는 `.gitignore`로 제외되어 있으므로 디바이스별로 별도 관리한다.
